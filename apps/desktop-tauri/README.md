...
The macOS arm64 release gate starts with an isolated, empty `XIAOHUI_APP_DATA_DIR` and deliberately unreachable Node/npm mirrors. It must provision bundled Node 22.19.0 and pnpm 11.7.0, complete the frozen offline install, and serve the workbench with HTTP 200. Acceptance checks require the XiaoHui product title, the Harbor, Codex Auth, Better Sidebar, and Personal Workbench client bundles in the Web boot manifest, and all product rows in Host configuration. The portable runtime reports Harbor 0.21.0 and discovers the `dsh-evolution` Python plugin. The Harbor acceptance path also verifies the frozen Host model binding, broker authorization, Docker-to-Host reachability, and a real Candidate response through GPT Auth.
