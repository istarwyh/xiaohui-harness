...
The Personal Workbench Client contributes a General settings card that loads the current native setting and detected macOS endpoints, then tests the desktop draft and the running Node Host separately against `https://chatgpt.com/`. The Node Host test requires process-local proof that the CLI installed its Dispatcher before it uses a fixed same-origin JSON POST and global `fetch`; its response contains only success, HTTP status, whether the proxy route is active, and a bounded transport code. Neither test requires or transmits a ChatGPT credential, and neither returns a proxy URL or raw request error to the browser. A draft whose proxy state differs from the active Host reports that a save, restart, and second test are required. Standalone `dsh web` renders the controls disabled because it has no trusted native owner.
